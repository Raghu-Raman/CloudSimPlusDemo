...
class BusTopology {
  ...
  val busLogger = CreateLogger(classOf[BusTopology])

  def start() = {
    busLogger.info(s"Starting execution of cloud models simulation.")
    ...
    busLogger.info("Intiating cloudlets")
    ...
    busLogger.info("Submitting VMs and cloudlets to broker")
    ...
    scalaCloudletList.map(cloudlet => {
      ...
      busLogger.info(s"Cost: $dc Cloudlet $cloudletId is $cost")
    }
    )
    busLogger.info(s"Finished simulation of bus topology simulation.")
  }
