...
class RingTopology {
  ...
  val ringLogger = CreateLogger(classOf[RingTopology])

  def start() = {
    ringLogger.info(s"Starting execution of cloud models simulation.")
    ...
    val NETWORK_BW = 30.0
    val NETWORK_LATENCY = 30.0
    // Initiate the topology.
    val networkTopology = new BriteNetworkTopology
    cloudSim.setNetworkTopology(networkTopology)
    networkTopology.addLink( broker,SaasDataCenter, NETWORK_BW, NETWORK_LATENCY)
    networkTopology.addLink( SaasDataCenter,IaasDataCenter,  NETWORK_BW, NETWORK_LATENCY)
    ...
    ringLogger.info("Initiating VMs")
    ...
    ringLogger.info("Intiating cloudlets")
    ...
    ringLogger.info("Submitting VMs and cloudlets to broker")
    ...
    scalaCloudletList.map(cloudlet => {
      ...
      ringLogger.info(s"Cost: $dc Cloudlet $cloudletId is $cost")
    }
    )
    ringLogger.info(s"Finished simulation of ring topology simulation.")
  }
